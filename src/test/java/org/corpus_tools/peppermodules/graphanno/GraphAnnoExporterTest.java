...
import org.corpus_tools.peppermodules.graphanno.GraphAnnoExporter;
 ...
 * {@link GraphAnnoExporter} class. Feel free to adapt and enhance this test class
...
public class GraphAnnoExporterTest extends PepperExporterTest {
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("GraphAnnoExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("GraphAnnoExporter").getAbsolutePath() + "/rootCorpus");
